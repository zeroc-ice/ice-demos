...
```shell
dotnet subscriber.dll --datagram
```
...
```shell
dotnet subscriber.dll --Clock.Subscriber.Endpoints="tcp -p <port> -h <host>"
```
...
```shell
dotnet subscriber.dll --retryCount <count>
```
...
3. Since the default configurations for the subscriber and IceStorm endpoints listen only on localhost, you must also
