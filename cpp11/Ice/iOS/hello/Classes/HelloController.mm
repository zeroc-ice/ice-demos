...
class HelloClient : public enable_shared_from_this<HelloClient>
{
public:
    ...
    void sayHello(const string& hostname, int deliveryMode, int timeout, int delay, bool discovery)
    {
        try
        {
               ...
               deliveryMode != DeliveryModeDatagramBatch)
            {
                ...
                _helloPrx->sayHelloAsync(delay, [self]() { self->response(); },
                                         [self](exception_ptr eptr) { self->exception(eptr); },
    ...
    void sent(bool sentSynchronously)
    {
        ...
        if(connection)
        {
            try
            {
                ...
                for(auto info = connection->getInfo(); info; info = info->underlying)
                {
    ...
    void shutdown(const string& hostname, int deliveryMode, int timeout, bool discovery)
    {
        try
        {
               ...
               deliveryMode != DeliveryModeDatagramBatch)
            {
                ...
                _helloPrx->shutdownAsync([self]() { self->response(); },
                                         [self](exception_ptr eptr) { self->exception(eptr); },
                                         [self](bool sentSynchronously) { self->sent(sentSynchronously); });
            }
            else
    ...
    void flushBatch()
    {
        ...
        try
        {
            ...
            _helloPrx->ice_flushBatchRequestsAsync([self](exception_ptr eptr) { self->exception(eptr); },
                                                   [self](bool sentSynchronously) { self->flushBatchSend(sentSynchronously); });
        }
        catch(const Ice::LocalException& ex)
        {
            exception(ex);
...
-(void)viewDidLoad
{
    ...
    client = make_shared<HelloClient>(self);
}
