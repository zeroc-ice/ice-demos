...
async def main():
    ...
    # an object adapter. We enable asyncio support by passing the current event loop to initialize. The communicator
    # gets its configuration properties from file config.client, in the client's current working directory. The
    # communicator initialization also parses the command-line options to find and set additional properties.
