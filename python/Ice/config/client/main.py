...
async def main():
    ...
    async with Ice.Communicator(initData=initData) as communicator:
        ...
        # If you run the server on a different computer, replace localhost in the Greeter.Proxy property in
        # client/client.conf with the server's hostname or IP address.
