...
async def main():
    ...
    with Ice.initialize(sys.argv, eventLoop=loop) as communicator:
        ...
        # Register the ConsolePrinter servant with the adapter, and get a proxy to the new object. We use a UUID for
        # the identity because these subscribers (weather stations) are transient: when a subscriber shuts down, we
        # want IceStorm to remove the subscription.
