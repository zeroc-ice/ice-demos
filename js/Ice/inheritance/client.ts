...
const rootDir = new Inheritance.DirectoryPrx(communicator, "RootDir:tcp -h localhost -p 4061");
 ...
 **/
async function listRecursive(dir: Inheritance.DirectoryPrx, depth: number) {
    ...
    for (const node of contents) {
        ...
        const subdir = await Inheritance.DirectoryPrx.checkedCast(node!);
        ...
        if (subdir != null) {
            ...
            const file: Inheritance.FilePrx = Inheritance.FilePrx.uncheckedCast(node!);
