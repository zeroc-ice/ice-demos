...
DEFAULT_CHANNEL = "3.8"
LANGUAGES = ["cpp", "csharp", "java", "js", "python", "swift"]
...
# Simpler NuGet config for C++ (no package source mapping needed)
CPP_NUGET_CONFIG_TEMPLATE = """\
<?xml version="1.0" encoding="utf-8"?>
<configuration>
  <packageSources>
    <add key="zeroc.com" value="{nuget_url}" />
...
def configure_cpp(channel: str) -> None:
    """Configure NuGet for C++ on Windows to use nightly feed."""
    urls = get_urls(channel)
    content = CPP_NUGET_CONFIG_TEMPLATE.format(nuget_url=urls["nuget"])
    dest = REPO_ROOT / "cpp" / "NuGet.Config"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Configure nightly feeds locally or in CI.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s                        # Configure all languages
  %(prog)s --language csharp      # Configure only C#
...
""",
    ...
    parser.add_argument(
        ...
        default="all",
        help="Language to configure (default: all)",
    )
    parser.add_argument(
        "-c",
        "--channel",
...
def reset_nightly() -> None:
    ...
    # Remove NuGet.Config files (C++ and C#)
    for nuget_dir in ["cpp", "csharp"]:
...
def main() -> int:
    ...
    # Always configure environment variables (needed for Python builds and C# IceVersion)
