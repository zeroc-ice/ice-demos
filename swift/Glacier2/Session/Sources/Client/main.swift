...
do {
    ...
    let router = try await communicator.stringToProxy("Glacier2/router:tcp -h localhost -p 4063")!
        .checkedCast(prx: Glacier2.RouterPrx.self)!
    ...
    var session = try await router.createSession(userId: userId, password: "password")

    // We configured a SessionManager on the Glacier2 router, so session is a non-null PokeSession.
    guard let pokeSession = session as? CatchThemAll.PokeSessionPrx else {
        fatalError("Session is not a PokeSession")
    }

    // Retrieve the PokeBox proxy from the session.
    guard let pokeBox = try await pokeSession.getPokeBox() else {
        fatalError("PokeBox is null")
    }

    var currentCount = try await pokeBox.getInventory().count
    ...
    try await pokeBox.caught(pokemon: newPokemon)
    ...
    session = try await router.createSession(userId: userId, password: "password")
    ...
    do {
        ...
        if error.replyStatus == .unauthorized {
