...
func run() -> Int32 {
    do {
        ...
        //
        guard let obj = try communicator.stringToProxy("RootDir:default -h localhost -p 10000") else {
            print("Invalid proxy")
            return 1
        }
        ...
        //
        guard let rootDir = try checkedCast(prx: obj, type: DirectoryPrx.self) else {
            print("Invalid proxy")
            return 1
        }
