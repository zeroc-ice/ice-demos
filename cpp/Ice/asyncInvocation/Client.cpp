...
run(const shared_ptr<Ice::Communicator>& communicator, const string& appName)
{
    ...
    // Calculate 10 - 4 with callback-based async operations which takes 'std::function's as callback parameters, the
    // first of which is called when the operation is completed
    {
        promise<int> p;
        calculator->subtractAsync(10, 4, [&p](int answer) { p.set_value(answer); });
    ...
    promise<int> a2;
    calculator->squareAsync(6, [&a2](int answer) { a2.set_value(answer); });

    promise<int> b2;
    calculator->squareAsync(8, [&b2](int answer) { b2.set_value(answer); });
    ...
    calculator->addAsync(
        ...
        {
            calculator->squareRootAsync(
                addResponse,
                [&hypotenuse](double squareRootResponse) { hypotenuse.set_value(squareRootResponse); });
        });
