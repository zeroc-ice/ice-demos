function client(args)
    %CLIENT An Ice client that recursively lists directory contents from a remote inheritance server.
    ...
    rootDir = inheritance.DirectoryPrx(communicator, 'RootDir:tcp -h localhost -p 4061');
...
function listRecursive(dir, depth)
    ...
    for i = contents
        ...
        subdir = inheritance.DirectoryPrx.checkedCast(node);
        ...
        if(~isempty(subdir))
            ...
            file = inheritance.FilePrx.uncheckedCast(node);
            lines = file.read();
            for j = lines
                fprintf('%s\t%s\n', indent, j{1});
            end
